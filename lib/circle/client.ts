...
const CIRCLE_API_BASE_URL =
  process.env.CIRCLE_API_BASE_URL || "https://api-sandbox.circle.com";
...
export class CircleClient {
  ...
  constructor() {
    ...
    console.log("🔧 Circle Client initialized:", {
      ...
      environment: this.apiKey.startsWith("TEST_API_KEY:")
        ? "sandbox"
        : "production",
    });
  }
  ...
  async getWireInstructions(bankAccountId: string, currency: string = "USD") {
    return this.request(
      `/v1/businessAccount/banks/wires/${bankAccountId}/instructions?currency=${currency}`
    );
  }
